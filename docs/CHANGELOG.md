...
- **显示布局**：新增窗口 1 全屏填充窗口 2 操作，启用后窗口 1 覆盖窗口 1/2 显示区域并隐藏窗口 2，关闭后恢复独立窗口
- **操作保护**：前端在跨屏状态下禁用窗口 2 相关播放、打开和显示设置按钮，避免隐藏窗口被误操作
- **预案持久化**：预案新增 `window1_fullscreen_to_window2` 状态，创建、编辑、保存当前状态和激活时均会同步恢复
- **API 解耦**：将播放与预案 REST API 从超 500 行视图文件拆分到独立模块，保留路由行为不变
