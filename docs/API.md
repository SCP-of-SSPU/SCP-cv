...
| `GET` | `/api/sources/{source_id}/ppt-resources/` | 无 | 获取 PPT 页资源、预览和提词器 |
| `PUT` | `/api/sources/{source_id}/ppt-resources/` | JSON: `resources` | 覆盖保存 PPT 页资源 |
...
PPT 资源对象字段：`id`、`source_id`、`page_index`、`slide_image`、`next_slide_image`、`speaker_notes`、`has_media`、`created_at`。该接口用于保存已解析出的幻灯片 PNG、下一页预览、提词器和页面媒体标记。
...
| `PATCH` | `/api/volume/` | JSON: `level` | 设置系统音量入口状态，范围 0-100 |
| `GET` | `/api/devices/` | 无 | 获取设备卡片状态 |
| `POST` | `/api/devices/{device_type}/toggle/` | 无 | 切换设备电源状态 |
| `POST` | `/api/devices/{device_type}/power/{action}/` | 无 | 设备开机或关机，`action` 为 `on` / `off` |

设备类型：`splice_screen`、`tv_left`、`tv_right`。设备响应包含 `is_placeholder` 和 `detail`，用于明确当前仅记录控制意图，尚未接入物理设备协议。音量响应包含 `system_synced` 和 `backend`，当前稳定入口为 `runtime_state`。静音策略固定为：窗口 3/4 始终静音；`single` 模式下窗口 2 静音；`double` 模式下窗口 1/2 不静音。
