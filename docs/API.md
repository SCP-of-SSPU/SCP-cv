...
| `POST` | `/api/sources/web/` | JSON: `url`, `name`, `folder_id`, `preheat_enabled`（兼容 `keep_alive`） | 通过网页 URL 创建媒体源 |
| `PATCH` | `/api/sources/{source_id}/` | JSON: `name`, `uri`, `preheat_enabled` | 更新媒体源名称、网页 URL 或预热开关 |
| `PATCH` | `/api/sources/{source_id}/move/` | JSON: `folder_id` | 移动媒体源到文件夹；空值表示根目录 |
| `GET` | `/api/sources/{source_id}/download/` | 无 | 下载文件型媒体源 |
| `GET` | `/api/sources/{source_id}/preview/` | 无 | 返回图片 / 视频源 inline 预览文件，供缩略图嵌入 |
...
| `PATCH` | `/api/sources/{source_id}/` | JSON: `name`, `uri`, `preheat_enabled`（兼容 `keep_alive`） | 更新媒体源可编辑字段 |
...
| `keep_alive` | bool | 旧版网页源保持活跃字段 |
| `preheat_enabled` | bool | 网页源预热开关，当前与 `keep_alive` 兼容同步 |
| `preview_url` | string | 可嵌入前端缩略位的预览 URL；无真实预览时为空字符串 |
| `thumbnail_url` | string | 缩略图 URL，当前与 `preview_url` 保持一致 |
| `preview_kind` | string | `image` / `video` / `icon`，指示前端渲染方式 |
| `preview_label` | string | 预览来源说明 |
| `created_at` | string | 创建时间 |

离线超过 1 小时的直播源会在源列表同步时自动删除。`folder_id=-1` 查询根目录源。临时上传源会记录过期时间，并在播放器切换或关闭该源后由播放器进程清理。
媒体源列表会直接返回缩略图信息：PPT 使用第一页 `slide_image`，图片 / 视频使用 `/api/sources/{source_id}/preview/`，网页和直播源无真实预览时回退类型图标。
网页源创建和编辑推荐使用 `preheat_enabled` 字段。启用后，播放器启动阶段会提前加载该网页源，切换到该源时复用已加载的 WebView；`keep_alive` 仅作为旧客户端兼容别名保留。

PPT 资源对象字段：`id`、`source_id`、`page_index`、`slide_image`、`next_slide_image`、`speaker_notes`、`has_media`、`media_items`、`created_at`。`media_items` 是当前页音视频对象数组，字段包括 `id`、`media_index`、`media_type`、`name`、`target`、`shape_id`。上传或注册 `.pptx` / `.ppsx` 时会尝试自动解析页数、备注和媒体引用；本机可用 PowerPoint COM 时会额外导出当前页/下一页 PNG 预览。`.ppt` / `.pps` 无法 zip 解析备注和媒体清单，但 PowerPoint COM 可用时仍会尝试生成预览资源。读取旧 PPT 源且资源为空时，接口会先尝试补齐页资源。
...
| `session_id` | int | 会话主键 |
| `source_id` | int/null | 当前源主键，无源时为空 |
