...
PPT 资源对象字段：`id`、`source_id`、`page_index`、`slide_image`、`next_slide_image`、`speaker_notes`、`has_media`、`media_items`、`created_at`。`media_items` 是当前页音视频对象数组，字段包括 `id`、`media_index`、`media_type`、`name`、`target`、`shape_id`。上传或注册 `.pptx` / `.ppsx` 时会尝试自动解析页数、备注和媒体引用；本机可用 PowerPoint COM 时会额外导出当前页/下一页 PNG 预览。`.ppt` / `.pps` 无法 zip 解析备注和媒体清单，但 PowerPoint COM 可用时仍会尝试生成预览资源。
