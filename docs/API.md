...
离线超过 1 小时的直播源会在源列表同步时自动删除。`folder_id=-1` 查询根目录源。
...
| `GET` | `/api/volume/` | 无 | 获取系统音量占位状态 |
| `PATCH` | `/api/volume/` | JSON: `level` | 设置系统音量占位状态，范围 0-100 |
...
设备类型：`splice_screen`、`tv_left`、`tv_right`。静音策略固定为：窗口 3/4 始终静音；`single` 模式下窗口 2 静音；`double` 模式下窗口 1/2 不静音。
