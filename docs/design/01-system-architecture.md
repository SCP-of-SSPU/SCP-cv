...
- 通过 Vue 控制台提供现场播控，统一使用 REST/SSE 作为控制与实时状态接口。
...
`runall --headless` 默认按窗口启动 4 个独立 PySide 播放器进程，隔离 PowerPoint COM 和 Qt 生命周期；直接 `run_player --headless` 也会在多窗口参数下拆分子进程，`--only-window` 可用于单窗口调试。
```
...
| 进程 | 启动入口 | 职责 | 迁移约束 |
| --- | --- | --- | --- |
| Django REST | `manage.py runserver`, `runall.py` | API、认证、服务层、数据库状态、SSE | 可迁移到目标 Django，但服务层语义要保留 |
...
| 显示目标为单屏 | 每个播放器窗口绑定一个物理显示器 | 左右拼接功能已从新合同移除 |
