...
def sync_streams_to_media_sources() -> dict[str, int]:
    ...
    from scp_cv.services.mediamtx import get_whep_read_url
    ...
    for stream in StreamSource.objects.filter(is_online=True):
        ...
        whep_url = get_whep_read_url(stream.stream_identifier)
        ...
        if existing is None:
            ...
            if not existing.is_available or existing.uri != whep_url:
                existing.is_available = True
                existing.uri = whep_url
