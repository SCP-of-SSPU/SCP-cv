...
<template>
  <aside class="ppt-slide-rail" :aria-label="t('pptRail.aria')">
    <header class="ppt-slide-rail__header">
      <span class="ppt-slide-rail__title">{{ t('pptRail.title') }}</span>
    ...
    <div class="ppt-slide-rail__list" role="list">
      <button
        ...
        :aria-label="t('pptRail.jumpTo', { n: item.pageIndex })"
      ...
      >
        ...
        <span class="ppt-slide-rail__meta">
          ...
          <span v-if="item.hasMedia" class="ppt-slide-rail__media" :aria-label="t('pptRail.hasMedia')" />
        </span>
      </button>

      <p v-if="items.length === 0" class="ppt-slide-rail__empty">{{ t('pptRail.empty') }}</p>
    </div>
  </aside>
</template>
