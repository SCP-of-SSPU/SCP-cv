...
def test_cleanup_residual_processes_only_terminates_project_owned_processes(
    monkeypatch: Any,
    tmp_path: Path,
) -> None:
    """
    重启兜底清理不得误杀无关 PowerShell，并应识别真实的 Python 播放器。
    ...
    class FakeResidualProcess:
        """提供 cleanup_residual_processes 所需的最小 psutil 进程接口。"""

        def __init__(self, process_info: dict[str, object]) -> None:
            """
            初始化模拟进程。

            :param process_info: psutil process_iter info 字典
            :return: None
            """
        ...
        def terminate(self) -> None:
            """记录项目进程被终止。"""
            terminated_processes.append(int(self.info["pid"]))
    ...
    processes = [
        ...
        FakeResidualProcess({
            ...
            "ppid": 1,
            "name": "mediamtx.exe",
            "cmdline": ["C:/tools/mediamtx.exe"],
            "cwd": "C:/tools",
            "exe": "C:/tools/mediamtx.exe",
        }),
        FakeResidualProcess({
            "pid": 23,
            "ppid": 1,
        ...
        FakeResidualProcess({
            ...
            "ppid": 1,
            "name": "mediamtx.exe",
            "cmdline": [str(project_dir / "tools/third_party/mediamtx/mediamtx.exe")],
            "cwd": str(project_dir),
            "exe": str(project_dir / "tools/third_party/mediamtx/mediamtx.exe"),
        }),
    ]
    monkeypatch.setattr(runall_processes.psutil, "process_iter", lambda _attrs: processes)
    monkeypatch.setattr(runall_processes.psutil, "pid_exists", lambda _pid: False)
    monkeypatch.setattr(
        runall_processes.psutil,
        "wait_procs",
    ...
    terminated = runall_processes.cleanup_residual_processes(
        current_pid=10,
        parent_pid=11,
        project_dir=project_dir,
    )

    assert terminated == [23, 24]
    assert terminated_processes == [23, 24]
